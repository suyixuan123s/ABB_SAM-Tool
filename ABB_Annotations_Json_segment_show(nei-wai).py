...
def extract_inner_contours(segmentation):
    '''
    从分割数据中提取所有内轮廓（除了最大外轮廓）
    '''
    if isinstance(segmentation, list) and len(segmentation) > 1:
        # 返回所有的轮廓，除了最大外轮廓
        return segmentation[1:] if segmentation else []
    return []
    ...
    for interior in interiors:
        if len(interior) >= 6 and len(interior) % 2 == 0:
            ...
            if exterior_path.contains_path(interior_path):
                # 从外轮廓中减去内轮廓
                polygon = patches.Polygon(interior_points, linewidth=1, edgecolor='black', facecolor='none')
